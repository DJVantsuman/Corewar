...
=======
	par[0] = (char)((*data)->program->map[(((*process)->position + 1)) % MEM_SIZE] & 192) >> 6;
	par[1] = (char)((*data)->program->map[(((*process)->position + 1)) % MEM_SIZE] & 48) >> 4;
	if (par[0] == DIR_CODE)
		val[0] = get_dir_value((*data), (*process), &shift, 4);
	else if (par[0] == IND_CODE)
		val[0] = get_ind_value((*data), (*process), &shift);
...
=======
	(*process)->carry = (*program)->carry == 0 ? 1 : 0;
>>>>>>> origin/master
