...
void 	load_value(t_data **data, t_process **process, unsigned int pos,
				   unsigned int val)
{
	(*data)->map[pos] = (char)((val & 4278190080) >> 24);
	(*data)->map[pos + 1] = (char)((val & 16711680) >> 16);
	(*data)->map[pos + 2] = (char)((val & 65280) >> 8);
	(*data)->map[pos + 3] = (char)((val & 255));
...
void    sti(t_data **data, t_process **process)
{
	...
	if (param[2] == REG_CODE)
		...
		return ;
	load_value(&(*data), &(*process), ((*process)->position + (val[1] +
